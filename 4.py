# log = "[1518-09-17 23:48] Guard #1307 begins shift"
# return int
def get_id(log):
    ans = ""
    i = log.index("#")
...
def get_minute(log):
    i = log.index(":")
    j = log.index("]")
    return int(log[i+1:j])

# return boolean
def does_shift_begin(log):
    return "begins" in log

# return boolean
def does_sleep(log):
    return "asleep" in log
...
def f1(logs):
    ...
    for i, log in enumerate(logs):
        if does_shift_begin(log):
            guard = get_id(log)
            if guard not in dic:
                dic[guard] = [0] * 1440
        elif does_sleep(log):
            start_min = get_minute(log)
            end_min = get_minute(logs[i + 1])
            for j in range(start_min, end_min):
                dic[guard][j] += 1;
    strategy_1 = float('-inf')
    m = max([sum(v) for v in dic.values()])
    for k, v in dic.items():
        if sum(v) == m:
            idx = v.index(max(v))
            strategy_1 = max(strategy_1, k * idx)

    strategy_2 = float('-inf')
    m = float('-inf')
    for k, v in dic.items():
        if max(v) > m:
            m = max(v)
            strategy_2 = v.index(m) * k
